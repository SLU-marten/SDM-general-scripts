...
species <- c("smallspottedCatshark" = "Scyliorhinus canicula", "thornbackRay" = "Raja clavata",
             "spinyDogfish" = "Squalus acanthias", "thornySkate" = "Amblyraja radiata")

## Select predictors from the loaded raster stack ####
predictors <-   c("bTemp", "bSal", "Depth", "botCur", "slope", "fishInt", "maxT")

## Load data ####
df <- readRDS("data/master_240311.rds") # The prepared data with catches and predictor values for each sample point
...
## Model parameters ####
timePeriod <- tibble("2005_2022" = c(2005, 2022),  "1972_2004" = c(1972, 2004)) # Which years should be included
quarter <- list("AllYear" = c(1,2,3,4), "Q1" = 1) # Which quarters should be included (Can be changed to months)
nRepPA_CV <- 5 # Number of repetitions of cross-validation points that will be drawn in the individual models
nRepPA_VI <- 5 # Number of permutations for to estimate variable importance in the individual models
nRepPAEns_VI <- 5 # Number of permutations for to estimate variable importance in the ensemble model
nRepAbu_SubSamp <- 5 # Number of subsampling in the abundance modeling
    ...
    for(sp in names(species)){
      ...
      tryCatch({
        ...
        myBiomodData <- BIOMOD_FormatingData(resp.var = dfPAFilt[sp], # Data.frame of species presence absence data
                                             ...
                                             resp.xy = dfPAFilt[, c("ShootLong", "ShootLat")], # Data.frame of the coordinates
        ...
        myBiomodModelOut <- BIOMOD_Modeling(bm.format = myBiomodData,
                                            ...
                                            models = c("GLM", "GBM", "GAM", "RF", "XGBOOST"), # Included individual model techniques
        ...
        writeRaster(sdm_E, paste0(pathAbu, sp, "/", sp, "_", names(timePeriod)[tp], "_", names(quarter)[q], "_Ensemble_model.tif"))

        ## DELTA ####
        # Create folder structure
        pathAbu <- paste(getwd(), "model output", modelID, "Delta/", sep = "/")
        if(!dir.exists(pathAbu)) dir.create(pathAbu, recursive = TRUE)
        ...
        writeRaster(delta_M, paste0(pathAbu, sp, "_", names(timePeriod)[tp], "_", names(quarter)[q],"_deltamodel.tif"))
      },
      error = function(e){
